...
class Handler(http.server.SimpleHTTPRequestHandler):
    def translate_path(self, path: str) -> str:
        ...
        if path == "/":
            path = "/index.html"
        # Only serve files that resolve inside dashboard/: a raw request for
        # "/../.env" must not reach the skill folder (and its API keys).
        candidate = (DASHBOARD_DIR / path.lstrip("/")).resolve()
        if not candidate.is_relative_to(DASHBOARD_DIR.resolve()):
            return str(DASHBOARD_DIR / "__not_found__")
